/**
 * Dashboard module — port of frontend /api/dashboard (KPIs, revenue chart,
 * patient growth, department distribution, status counts, stock alerts).
 * Tenant/branch isolation via lib/prisma.ts.
 */
  ...
  try {
    ...
    const [patients, doctors, todayAppts, todayInvoices, allInvoices, labTests, medicines, prescriptions, recentActivities, weekInvoices, weekAppointments, monthlyPatients, depts] =
      await Promise.all([
        ...
        db.invoice.findMany({
          where: { date: { gte: last7Days[0].gte, lt: last7Days[6].lt }, ...branchFilter },
        ...
        db.patient.findMany({
          where: { registeredAt: { gte: last6Months[0].gte, lt: last6Months[5].lt }, ...branchFilter },
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      const dayInvoices = weekInvoices.filter((inv) => inv.date.toDateString() === dayStr);
      const dayAppointments = weekAppointments.filter((apt) => apt.date.toDateString() === dayStr);
    ...
    const patientGrowth: { month: string; patients: number }[] = last6Months.map(
      (range) => ({
        ...
        patients: monthlyPatients.filter(
          (p) => p.registeredAt >= range.gte && p.registeredAt < range.lt
        ).length,
      })
    );
