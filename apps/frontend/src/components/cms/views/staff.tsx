  ...
  const submit = async () => {
    ...
    try {
      const payload: Record<string, unknown> = {
        ...
        branchIds: form.branchIds.length > 0 ? form.branchIds : (defaultBranchId ? [defaultBranchId] : []),
      };
      ...
      toast.error(e?.message || (isEdit ? "Failed to update staff" : "Failed to add staff"));
    } finally {
      setSaving(false);
    }
  };
