...
async function main() {
  ...
  // Roles
  const adminRole = await db.role.create({ data: { name: "Super Admin", description: "Full system access", permissions: { create: Object.values(permMap).map((id) => ({ permissionId: id })) } } });
  const doctorRole = await db.role.create({ data: { name: "Doctor", description: "Clinical access", permissions: { create: ["Dashboard.view", "Patient.view", "Patient.create", "Patient.edit", "Appointment.view", "Appointment.edit", "Prescription.view", "Prescription.create", "EMR.view", "EMR.create", "EMR.edit", "Laboratory.view", "Laboratory.create", "Radiology.view", "Billing.view", "Patient.print", "Prescription.print"].map((k) => ({ permissionId: permMap[k] })) } } });
  const receptionRole = await db.role.create({ data: { name: "Receptionist", description: "Front desk operations", permissions: { create: ["Dashboard.view", "Patient.view", "Patient.create", "Patient.edit", "Appointment.view", "Appointment.create", "Appointment.edit", "Billing.view", "Billing.create", "Patient.print", "Appointment.print"].map((k) => ({ permissionId: permMap[k] })) } } });
  await db.role.create({ data: { name: "Pharmacist", description: "Pharmacy operations", permissions: { create: ["Dashboard.view", "Pharmacy.view", "Pharmacy.create", "Pharmacy.edit", "Inventory.view", "Billing.view", "Pharmacy.print", "Pharmacy.export"].map((k) => ({ permissionId: permMap[k] })) } } });
  await db.role.create({ data: { name: "Lab Technician", description: "Laboratory operations", permissions: { create: ["Dashboard.view", "Laboratory.view", "Laboratory.create", "Laboratory.edit", "Radiology.view", "Laboratory.print"].map((k) => ({ permissionId: permMap[k] })) } } });
  await db.role.create({ data: { name: "Accountant", description: "Finance operations", permissions: { create: ["Dashboard.view", "Billing.view", "Billing.create", "Billing.edit", "Reports.view", "Reports.export"].map((k) => ({ permissionId: permMap[k] })) } } });
